...
def takeStep(app):
    ...
    netForces = dict() #dictionary mapping each body to the net force acting on it feeling
    ...
    for i in range(len(Body.instances)):
        ...
        for j in range(i+1,len(Body.instances)):
            ...
            if r.mag > (cBod1.radius + cBod2.radius):
                ...
                print(cBod1.netForceFelt)
                print(cBod2.netForceFelt)
                print(' ')
    ...
    for i in range(len(Body.instances)):
        cBod = Body.instances[i]

        cBod.momentum = cBod.momentum + (cBod.netForceFelt * app.dt)
        #print(cBod.momentum, cBod.netForceFelt, cBod.netForceFelt*app.dt)
